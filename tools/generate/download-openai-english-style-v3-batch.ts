...
async function main(): Promise<void> {
  ...
  if (metadata.status !== "completed") {
    throw new Error(
      `English style-v3 batch is not completed. Current status: ${metadata.status}. Check it first.`
    );
  }

  if (!metadata.outputFileId) {
    throw new Error("English style-v3 metadata does not contain outputFileId.");
  }
  ...
  console.log("Downloaded English style-v3 batch output.");
  console.log("Raw output: data/explanations/english-style-v3-batch-1/openai-explanation-batch-raw-output.jsonl");
  console.log(
    metadata.errorFileId
      ? "Downloaded the batch error file."
      : "No batch error file was present."
  );
}
